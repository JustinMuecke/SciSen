...
if __name__ == '__main__':
    ...
    # download options
    downloadurl_parser = cmd_parsers.add_parser("downloadURL", help="download raw data")
    downloadurl_parser.add_argument('--sleep', default=2, required=False, help='sleep time between two burst')
    downloadurl_parser.add_argument('--burst', default=2, required=False, help='size of one reading burst')
    #downloadurl_parser.add_argument('--in', '-i', help='the path to the file with the papers to be crawled')
    downloadurl_parser.add_argument('--proxy', default=False, required=False, help='allows the use of proxys when connection gets blocked', action='store_true')
    downloadurl_parser.add_argument('--diff', default=False, required=False, help='rescans the directory before starting crawling to determine which files already have been downloaded',
                                 action='store_true')
    ...
    elif "downloadURL" == used_command:
        asyncio.run(data.getDataFromURLs(arguments["out"], int(arguments["sleep"]), int(arguments["burst"]), arguments["proxy"], arguments["diff"]))
