...
def replaceUrl(url, use_proxy):
    return url.replace("arxiv.org", exportUrl).replace('abs', 'e-print').replace('https','http').replace('http', 'https') #prevent httpss://
...
#clone of getData() with list of [arxiv_id, url]
async def getDataFromURLs(dest, sleepLength, burstSize, proxy, diff):#TODO add urls,
    # check if destination folder exists else create
    if not path.exists(dest):
        os.mkdir(dest)
    ...
    #job_queue = getPaperUrlsByCategory([10, 62, 122], start_id, stop_id)
    # TODO url list with arxiv_id #currently only a work around
    job_queue = pd.read_csv(r'papers_to_crawl.csv').values.tolist()
    #job_queue = [['12321.123','http://arxiv.org/abs/1805.10616v4'],['123123.1231','http://arxiv.org/abs/1805.10616v4']]#urls
    already_existing_ids = list()

    if diff: #TODO test if it does work here
        already_existing_ids = get_ids_from_disk(dest)
        old = len(job_queue)
        print("Fetching already existing paper entries. Found ", len(already_existing_ids))
        job_queue = fetch_ordered(job_queue, already_existing_ids)
    ...
    try:
        while requestCount < len(job_queue):
            ...
            for i in range(0, burstSize):
                record = job_queue[requestCount]
                requestCount += 1
                url = replaceUrl(record[1], len(curr_proxy) > 4)
                currUrls.append((url, record[0], record))
            ...
            for (url, id, r) in currUrls:
                status, st_record = await downloadPaperLegacy(dest, url, id, r)
                if status is not Status.OK:
                    job_queue.append(st_record)#TODO just url
                    print("Adding {} again to list to query it later".format(st_record[0]))
                    newProxyNeeded = False
                if status is Status.OK:
                    paperCounter.increment(1)
...
async def downloadPaperLegacy(dest, url, identifier, tuple):
    ...
    if r.status_code == 200:
        ...
        # remove '.' in identifier otherwise the text conversion won't work properly
        savePath = os.path.join(dest, str(identifier).replace('.','') + ext)
